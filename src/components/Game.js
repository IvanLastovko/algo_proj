...
export const Game = () => {
    ...
    const words = {
        ...
        '3': {
            ...
            "words": ['cat.', '896.', '?=+.', 'abc1'],
            "matches": [true, true, true, false],
            "explanations": ["Regex engine supports characters from ASCII table. Except letters and digits regex may use special characters for pattern matching. Some of these characters called metacharacters and they may perform special operations in pattern searching. These characters are backslash \\, the caret ^, the dollar sign $, the period ., the vertical bar |, the question mark ?, the Kleene star *, the plus sign + , the opening and closing parenthesis ( ), the opening and clothing square brackets [ ], and the opening and clothing curly braces { }. The operations connected with these characters will be in next chapters. In order to use them for pattern matching these characters must contain backslash before them. Backlash allows to use metacharacters as usual characters. For example, the pattern ‘\\( 1 \\+ 2 \\) \\*2 = 6’ would have a match in the string “12 - (1+2)*2 = 6”."]
        },
        '4': {
